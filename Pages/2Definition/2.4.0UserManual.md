...
The user also needs to be in possession of an email account capable of using IMAP.
...
To navigate to the course view the user has to use  the sidebar menu. In the course view, you can switch in between the courses and see which students are attending. It is also possible to select a past date to view the course's attendance there. The names of the students are listed in a table and the missing students are highlighted in red. A dropdown menu in the right upper corner allows the user to view, edit or add a list to the currently viewed course.

To get to the student view the user will have to use the sidebar menu. In this case, the user selects the course using a dropdown menu after which the chosen course list appears from which the user can click on or search for the student. The single student view shows all absences in numeric order. The latest absences are shown first.

The settings view can be reached over its button in the sidebar menu. Another sidebar menu opens where the different subpages are listed. The option "Email" allows the user to access and modify the currently used mail sever. YAAC's Wiki pages are linked under "Wiki" and the legal information contains details about data security, regulations and a contact of the project team.
