...
The default E-Mail backend implementation implements the `MailClient` interface in `internal/shared/mvvm.go`.
...
### Intent/Responsibility
The frontend has the responsibility of providing a single graphical interface for the user which is easy to use without any technical knowlage. Further it gives the user feedback on the background tasks and notifying them if an error occured.
