...
The three subsystems mail integration, image processing and database form the backend.
The following diagram shows a general overview of how they work together.

![BackendActivityDiagram](/Diagrams/Images/BackendActivityDiagram.png)
...
### 1.2 Interfaces
The mail client functionality is provided by the `MailClient` interface. The default MVVM implementation inherits its database functionality from `BackendMail`, which is the implementation of `MailClient`.
This backend fetches and analyses the mail data and the data in it can only be accessed through the interface provided by the backend.
...
|UpdateMailCredentials|Update the E-Mail server address and the credentials to use for future connections to the mail server|
...
A more detailed of this submodule can be found [here](/Pages/3Design/3.4.0ImageProcessing.md).
...
The default image processing backend implementation implements the `ImgProc` interface in `internal/shared/mvvm.go`.
...
The default database backend implementation implements the `DatabaseClient` interface in `internal/shared/mvvm.go`.
...
The frontend has the responsibility of providing a single graphical interface for the user which is easy to use without any technical knowlage. Further it gives the user feedback on the background tasks and notifying them if an error occured. It enables the user to access the provided data and to change it if necessary. It presents the data and functionalities in a clear and well structured user interface.

### 4.2 Interfaces

The default frontend implementation implements the `Frontend` interface in `internal/shared/mvvm.go`.

|**Method**|**Short description**|
|---|---|
|NotifyNewList|This function is called by the backend when a new list arrived, so that it can notify the user.|
|NotifyError|Notify the frontend if an error, that inhibits the completion of a task, has occured in the backend.|

### 4.3 Files

The code for this module can be found in `internal/frontend` and `internal/mvvm/frontend.go`.
