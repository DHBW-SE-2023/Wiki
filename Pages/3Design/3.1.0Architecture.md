...
As outlined in [1. HMI](#1-hmi) the software was originally intended to run on every operating system without additional installation steps or dependencies but this did not work due to the GOCV library dependency not being able to link statically.

To solve the compatibility issues and to make running the final product as easy as possible `Docker` was choosen as an alternative, the image providing the necessary dependencies and abstracting the users operating system.
...
- gosseract: binding to Tesseract C++ library used for optical character recognition allows to get the student names out of lists
- gocv: wrapper for the OpenCV library used for image recognition of the student lists
...
Cross-plattform UI toolkit for GO using the GTK toolkit. It allows to define the user interface using GO and export the it in one executable file including backend and frontend on every desktop and even mobile platform without any other software required on the endusers system. The decision to use fyne is outlined in [0.0.0 Research Phase](../0Research/0.0.0ResearchPhase.md).
<!--
